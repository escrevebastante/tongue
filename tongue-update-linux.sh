...
#HELM DOCUMENTATION
git init $DOCUMENTPATH/helm.sh
git -C $DOCUMENTPATH/helm.sh sparse-checkout set content/en/docs
git -C $DOCUMENTPATH/helm.sh config core.sparseCheckout true
git -C $DOCUMENTPATH/helm.sh reset --hard
git -C $DOCUMENTPATH/helm.sh pull https://github.com/helm/helm-www.git
...
grep -rl --include="*.html" "<head>" $DOCUMENTPATH/* | while read file; do
  ...
  if html2text -b 999999 "$file" utf8 >"$output"; then
